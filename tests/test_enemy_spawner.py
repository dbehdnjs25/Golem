...
def test_spawns_after_interval():
    ...
    result = spawner.update(
        config.GOLEM_SPAWN_INTERVAL, enemies, pygame.Vector2(10, 10), _core(), random.Random(1)
    )
...
def test_respects_max_cap():
    ...
    enemies = [Golem(pos=pygame.Vector2(10, 10))]
    result = spawner.update(
        config.GOLEM_SPAWN_INTERVAL, enemies, pygame.Vector2(10, 10), _core(), random.Random(1)
    )
...
def test_spawn_avoids_core_sync_zone_and_player():
    ...
    v = spawner.update(config.GOLEM_SPAWN_INTERVAL, enemies, player_pos, core, random.Random(7))
