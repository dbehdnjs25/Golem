...
코어를 거의 0에서 되살리려면 파편 80개가 든다. 저장고 용량이 750 MB이고 파편이 3 MB이므로
250개를 담을 수 있으니, 완전 회복은 저장고의 **약 3분의 1**을 먹는다. 한 번 크게 뚫리면
보스 소환 자금이 눈에 띄게 줄지만 판이 끝나지는 않는다.

이 비율은 **이 스펙에서 가장 자신 없는 숫자**다. 초안에서는 저장고가 100개라 완전 회복이
80 %를 먹었는데 그건 너무 가혹해 보였고, 인벤토리 스펙(E)에서 용량이 250개로 늘면서 지금
값이 됐다. 플레이해보고 제일 먼저 조정할 항목이다.
